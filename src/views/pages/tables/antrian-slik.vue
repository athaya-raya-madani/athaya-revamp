<script setup>
  ...
  const getAntrian = async () => {
    try {
      // fetch data
      const response = await api.get('/api/permohonan/verifikasi-slik');
      ...
      <tr v-else v-for="(verifikasi, index) in verifikasiData" :key="index">
        ...
        <td>
          <div class="d-flex justify-content-center">
            <VBtn :to="{ name: 'antrianslik.edit', params: { id: verifikasi.id } }" class="v-btn btn-sm bg-warning rounded-sm shadow border-0 me-2">
              Lihat
            </VBtn>
          </div>
        </td>
